...
$runnerTableSettings['transactions_lcsc'] = array(
...
prop_address,
docuserid,
cast(AES_DECRYPT(unhex(cea_reg), \'$jw6857r707\') as char) AS cea_reg,
cobroke_comm,
agency_fee,
commission,
sale_price,
...
str_filing_date,
remarks,
cast(AES_DECRYPT(unhex(salesperson_email), \'$jw6857r707\') as char) AS salesperson_email,
comm_to_agency,
...
client_nric,
agency,
cast(AES_DECRYPT(unhex(agency_cea), \'$jw6857r707\') as char) AS agency_cea,
management_fee,
commencement_date,
end_date,
comm_to_agency1,
...
WHERE (client =\'LIM CHIN SAN CONTRACTORS PTE LTD\')
GROUP BY id, client
ORDER BY `date` DESC, ID_Tx DESC',
	'keyFields' => array( 
		'id' 
	),
	...
	'fields' => array(
		...
		'salesperson_name' => array(
			...
			'editFormats' => array(
				'edit' => array(
					...
					'lookupWhere' => 'status=\'SALESPERSON\'',
					'lookupDependentFields' => array( 
						array(
							'masterField' => 'cea_reg',
							'lookupField' => 'cea_reg' 
		...
		'cea_reg' => array(
			...
			'index' => 11,
			'type' => 201,
			'sqlExpression' => 'cast(AES_DECRYPT(unhex(cea_reg), \'$jw6857r707\') as char)',
			'uploadFolder' => '',
...
$value="<font color=\'$color\'>$value</font>";',
		...
		'salesperson_email' => array(
			...
			'index' => 75,
			'type' => 201,
			'sqlExpression' => 'cast(AES_DECRYPT(unhex(salesperson_email), \'$jw6857r707\') as char)',
			'viewFormats' => array(
				'view' => array(
					'imageWidth' => 457,
					'imageHeight' => 0,
...
$value = implode(",",$arr);',
		...
		'agency_cea' => array(
			...
			'index' => 85,
			'type' => 201,
			'sqlExpression' => 'cast(AES_DECRYPT(unhex(agency_cea), \'$jw6857r707\') as char)',
			'viewFormats' => array(
				'view' => array(
					'imageWidth' => 457,
					'imageHeight' => 0,
...
ORDER BY `date` DESC, ID_Tx DESC',
		...
		'fieldList' => array( 
			array(
				...
				'expression' => array(
					...
					'parsed' => true,
					'type' => 'SQLField',
					'table' => 'jwresour_jwr_saas_db_jwrealty.transactions',
					'name' => 'id' 
				),
				'encrypted' => false,
				'columnName' => 'id' 
			...
			array(
				'sql' => 'cast(AES_DECRYPT(unhex(cea_reg), \'$jw6857r707\') as char)',
				'parsed' => true,
				'type' => 'FieldListItem',
				'alias' => 'cea_reg',
				'expression' => array(
					...
					'arguments' => array( 
						array(
							'sql' => 'AES_DECRYPT(unhex(cea_reg), \'$jw6857r707\') as char',
							'parsed' => true,
			...
			array(
				'sql' => 'cast(AES_DECRYPT(unhex(salesperson_email), \'$jw6857r707\') as char)',
				'parsed' => true,
				'type' => 'FieldListItem',
				'alias' => 'salesperson_email',
				'expression' => array(
					'sql' => 'cast(AES_DECRYPT(unhex(salesperson_email), \'$jw6857r707\') as char)',
					'parsed' => true,
					'type' => 'FunctionCall',
					'arguments' => array( 
						array(
							'sql' => 'AES_DECRYPT(unhex(salesperson_email), \'$jw6857r707\') as char',
							'parsed' => true,
							'type' => 'NonParsedEntity' 
						) 
					),
			...
			array(
				'sql' => 'cast(AES_DECRYPT(unhex(agency_cea), \'$jw6857r707\') as char)',
				'parsed' => true,
				'type' => 'FieldListItem',
				'alias' => 'agency_cea',
				'expression' => array(
					...
					'arguments' => array( 
						array(
							'sql' => 'AES_DECRYPT(unhex(agency_cea), \'$jw6857r707\') as char',
							'parsed' => true,
							'type' => 'NonParsedEntity' 
						) 
					),
		...
		'fromList' => array( 
			array(
				'sql' => 'jwresour_jwr_saas_db_jwrealty.transactions',
				'parsed' => true,
				'type' => 'FromListItem',
				'table' => array(
					'sql' => 'jwresour_jwr_saas_db_jwrealty.transactions',
					...
					'name' => 'jwresour_jwr_saas_db_jwrealty.transactions' 
				),
				'joinOn' => array(
					'sql' => '',
					'parsed' => false,
		...
		'where' => array(
			'sql' => 'client =\'LIM CHIN SAN CONTRACTORS PTE LTD\'',
			'parsed' => true,
			'type' => 'LogicalExpression',
			'contained' => array( 
			...
			'case' => '=\'LIM CHIN SAN CONTRACTORS PTE LTD\'',
			'useAlias' => false 
		),
		'groupBy' => array( 
			array(
...
alertlist_cust,
alertlist_result',
		'fromListSql' => 'FROM jwresour_jwr_saas_db_jwrealty.transactions',
		'whereSql' => '(client =\'LIM CHIN SAN CONTRACTORS PTE LTD\')',
		'orderBySql' => 'ORDER BY `date` DESC, ID_Tx DESC',
		'tailSql' => '' 
	),
	'hasEvents' => true,
	'hasJsEvents' => true,
...
if( mlang_getcurrentlang() === 'English' ) {
	$runnerTableLabels['transactions_lcsc'] = array(
	'tableCaption' => '11 Lor 4 Geylang Leases',
	'fieldLabels' => array(
		'id' => 'ID',
		...
		'comm_earned_salesperson' => 'Gross Commission',
		...
		'status_status' => 'Transaction Status',
		...
		'comm_nett_salesperson' => 'Nett Comm to Salesperson',
		...
		'namelist_cust' => 'Customer-namelist',
		'alertlist_cust' => 'Customer-alertlist',
		'alertlist_result' => 'Alert list' 
	),
	'fieldTooltips' => array(
		'id' => '',
		'category' => 'Transaction type cannot be changed. To chage type, delete transaction and re-create new transaction.',
		'prop_type' => '',
		'type_of_property' => '',
		'Prop_kind' => '',
		'rental_price' => '',
