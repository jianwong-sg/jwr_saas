<?php
class eventclass_transactions_sale  extends TableEventsBase {
...
$strWhereClause = whereAdd($strWhereClause,"category='SALE'");

/*
$action = postvalue("a");

    // Only run filter if not in search or show all
    if ($action != "search" && $action != "advsearch" && $action != "showall") {

        // Build the filter
        $filter = "category='SALE'";

        // Apply filter differently for Guests to bypass security clause conflicts
        if (!isLogged()) {
            if (stripos($strSQL, "WHERE") !== false) {
                $strSQL .= " AND " . $filter;
            } else {
                $strSQL .= " WHERE " . $filter;
            }
        } else {
            // For logged in users, normal way
            $strWhereClause = whereAdd($strWhereClause, $filter);
        }
    }
