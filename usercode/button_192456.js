Runner.buttonEvents['192456'] = function( pageObj, proxy, pageid ) {
	var gid = '192456';
	...
	if ( !pageObj.buttonEventBefore[ gid ] ) {
		pageObj.buttonEventBefore[ gid ] = function( params, ctrl, pageObj, proxy, pageid, rowData, row, submit ) {		
			var ajax = ctrl;
			//location.href = "transactions_edit.php?editid1='{$editlink}'";
//return false;			
		}
	}
	...
	if ( !pageObj.buttonEventAfter[ gid ] ) {
		pageObj.buttonEventAfter[ gid ] = function( result, ctrl, pageObj, proxy, pageid, rowData, row, params ) {
			var ajax = ctrl;
			//if (result.record["title"]=='ESTATE AGENCY AGREEMENT FOR THE SALE OF RESIDENTIAL PROPERTY')
location.href='transactions_edit.php?editid1='+result["id"];
		}
	}
