<?php
class eventclass_form3  extends TableEventsBase {
...
// Mapping fields for landlords and interpreters
$fields = [
...
foreach ($fields as $signField => $dateField) {
    if ($values[$signField] != $oldvalues[$signField]) {
        $statusCondition = strpos($signField, 'interpreter') !== false ? 'status = 3 AND ' : '';
...
{ 
   $pageObject->hideItem("view_signed_form3", $recordId); 
} 

if ($data["landlord1"]=="") 
{ 
   $pageObject->hideItem("esign_form3", $recordId); 
	 $pageObject->hideItem("view_unsign_form3", $recordId); 
}
		;
