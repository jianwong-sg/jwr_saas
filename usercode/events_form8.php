<?php
class eventclass_form8  extends TableEventsBase {
...
{ 
   $pageObject->hideItem("view_signed_form8", $recordId); 
} 

if ($data["tenant1"]=="") 
{ 
   $pageObject->hideItem("esign_form8", $recordId); 
	 $pageObject->hideItem("view_unsign_form8", $recordId); 
}
		;
...
// Mapping fields for tenants, interpreters, and agent
$fields = [
...
foreach ($fields as $signField => $dateField) {
    if ($values[$signField] != $oldvalues[$signField]) {
        $statusCondition = strpos($signField, 'interpreter') !== false ? 'status = 8 AND ' : '';
...
public function default_interpret1_efedit(  ) {
	$defaultValue = "Nil";
return $defaultValue;
}

public function default_interpret2_efedit(  ) {
	$defaultValue = "Nil";
return $defaultValue;
}

public function default_interpret3_efedit(  ) {
	$defaultValue = "Nil";
return $defaultValue;
}

public function default_interpret4_efedit(  ) {
	$defaultValue = "Nil";
return $defaultValue;
}
