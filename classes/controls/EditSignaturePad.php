...
class EditSignaturePad extends UserControl
{
	...
	public function buildUserControl($value, $mode, $fieldNum = 0, $validate, $additionalCtrlParams, $data)
	{
		...
		$_SESSION["db_bgimage_".$this->cfield] = base64_encode($value);
		if($value){
			...
			if($this->target!="db" || !$_SESSION["db_bgimage_".$this->cfield]){
	...
	{
		$field_name = $this->goodFieldName."_".$this->id;
		if($_SESSION["delsigfile_".$field_name])
			$_SESSION["db_bgimage_".$this->cfield] = "";
		if($this->target!="db" || !$_SESSION["db_bgimage_".$this->cfield]){
			if(file_exists($_SESSION["file_".$this->cfield])){
				$bgimage = $_SESSION["file_".$this->cfield];
				$this->bgcolor = "transparent";
			...
			$bgimage = $_SESSION["db_bgimage_".$this->cfield];
			if($bgimage)
				$this->bgcolor = "transparent";
		}
		...
		elseif($_SESSION["delsigfile_".$field_name]){
			$avalues[ $this->field ] = "";
		}
		$_SESSION["delsigfile_".$field_name] = "";
		//unlink($bgfilename);
	}
}
?>